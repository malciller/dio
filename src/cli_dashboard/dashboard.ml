 ...
 *)
 ...
 let get_balance_info () : balance_info list Lwt.t =
   ...
   let balance_info_list_lwt = Lwt_list.fold_left_s (fun acc asset ->
     ...
     | Some price_usd ->
        ...
        (* For USDG: when not in sell orders, it should be considered fully accumulated as it auto-moves to earn *)
        let raw_accumulated_balance =
            total_balance -. qty_on_sell_orders
        in
 ...
 let render state =
   ...
   let key_bindings_img = I.hcat [
       ...
       I.string key_bracket_style "│";
       I.string key_bracket_style " ["; I.string key_text_style "T"; I.string key_bracket_style "]";
       I.string text_style "elemetry ";
       I.string key_bracket_style "│";
   ...
   let asset_rows_section_with_boxing =
     if not state.show_assets || state.show_telemetry then
       I.empty
     else
       match asset_rows with
       | [] -> I.empty
   ...
   let balances_section_image =
     if state.show_balances && not state.show_telemetry then
   ...
   let other_height = 1 (* header *) + 1 (* void after header *) +
                    (if state.show_balances && state.balances <> [] && not state.show_telemetry then balances_section_height + 1 (* void after balances *) else  0) +
                    (if state.show_assets && not state.show_telemetry then 1 (* void before assets *) + new_asset_rows_height + 1 (* void after assets *) else 0) in

  (* Calculate telemetry height - it should share space with logs when both are shown *)
  let telemetry_height =
    if state.show_telemetry then
      let available_for_telemetry = term_height - (1 (* header *) + 1 (* void after header *) +
                                      (if state.show_balances && state.balances <> [] && not state.show_telemetry then balances_section_height + 1 else 0) +
                                      (if state.show_assets && not state.show_telemetry then 1 + new_asset_rows_height + 1 else 0))
      in
      min 15 (max 5 available_for_telemetry)  (* Reserve reasonable space for telemetry *)
    else 0
  in

  let logs_height =
    if state.show_logs then
      max 0 (term_height - other_height - telemetry_height - (if state.show_telemetry then 1 else 0))
    else 0
  in
...
let telemetry_panel =
  if state.show_telemetry then
    Telemetry_view.render_telemetry_panel_preloaded term_width telemetry_height state.telemetry_stats
  else
    I.empty
in
 ...
 I.vcat [
   ...
   I.void term_width 1;
   telemetry_panel;
   (if state.show_telemetry then I.void term_width 1 else I.empty);
   balances_section_image;
   (if state.show_balances && state.balances <> [] && not state.show_telemetry then I.void term_width 1 else I.empty);
   (if state.show_assets && not state.show_telemetry then asset_rows_section_with_boxing else I.empty);
   (if state.show_assets && not state.show_telemetry then I.void term_width 1 else I.empty);
   logs_section_image;
   I.void term_width 1
 ]
 ...
 let start ~on_quit:(on_quit: unit -> unit Lwt.t) () : Notty_lwt.Term.t =
   ...
   let handle_event = function
     ...
     | `Key (`ASCII 'a', []) | `Key (`ASCII 'A', []) ->
         ...
         Lwt.return_unit
     | `Key (`ASCII 't', []) | `Key (`ASCII 'T', []) ->
         state := { !state with show_telemetry = not !state.show_telemetry };
         Lwt.return_unit
     | _ -> Lwt.return_unit
   in
 ...
 let rec tick () =
   ...
   Telemetry.get_all_stats () >>= fun telemetry_stats ->
